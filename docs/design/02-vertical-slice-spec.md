...
| **TIME / SECOND MVP** | TIME meter measured in SECOND, earn SECOND from a small objective, spend TIME on one useful service, zero TIME triggers reincarnation placeholder |
...
| **NFT Hunter Frame skin** | 1 skin equip flow + escrow contract (test net DOS Chain) |
...
- [ ] At least one playtester notices TIME / SECOND as a meaningful tradeoff, not just a timer
...
- [ ] Per-system GDDs in `docs/design/` for Combat, AI agent, Reincarnation, TIME / SECOND economy, NFT escrow, LLM NPC. (TIME / SECOND is drafted; advanced body progression is deferred.)
...
| 2. Networked player + zone | T+1 to T+4 | 1 zone Photon Fusion 2 multiplayer, Hunter Frame skin spawn, minimal ARPG controller first, Opsive UCC evaluated after baseline |
...
| 5. Reincarnation + level/stat persistence | T+12 to T+16 | Death -> SECOND -> reincarnation flow, current-body reset, profile/stat persistence |
| 6. TIME / SECOND economy | T+16 to T+18 | TIME meter measured in SECOND, one earn source, one spend sink, zero-time reincarnation trigger |
...
- **PIVOT** to revised design if: technical works but core loop falls flat (e.g., AI agent autoplay feels invisible, reincarnation feels punishing, TIME feels like a nuisance timer).
