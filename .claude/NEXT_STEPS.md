...
Create empty folders so Unity project root is consistent. Run:

```powershell
cd D:\Projects\Second-Spawn
New-Item -ItemType Directory -Force -Path Assets, Assets\Scripts, Assets\Scripts\Gameplay, Assets\Scripts\Networking, Assets\Scripts\AI, Assets\Scripts\UI, Assets\Scripts\NFT, Assets\Art, Assets\Audio, Assets\Prefabs, Assets\Scenes, Assets\Settings, Packages, ProjectSettings, docs\design, backend\gateway, .claude\commands, .claude\templates | Out-Null
```
...
3. Project location: `D:\Projects\Second-Spawn`
4. Project name: `Second-Spawn`
...
4. Configure Photon App ID in `Assets/Photon/Fusion/Resources/PhotonAppSettings.asset`
...
3. Service role key goes to backend `.env`, anon key OK in Unity `Assets/Settings/SupabaseConfig.asset` (anon is public-safe)
